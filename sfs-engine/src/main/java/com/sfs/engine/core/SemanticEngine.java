...
import com.sfs.engine.inspect.FileInspector;
...
public final class SemanticEngine implements AutoCloseable {
    ...
    public SemanticEngine(ContentSource contentSource,
                          InMemorySemanticRecordStore recordStore,
                          AnalysisCache cache,
                          AnalysisLevelPolicy levelPolicy,
                          AnalysisCompletionListener completionListener,
                          Clock clock) {
        this.contentSource = Objects.requireNonNull(contentSource, "contentSource must not be null");
    ...
    private AnalysisJob runJob(AnalysisJob queued) {
        markRunning(queued);
        try {
            byte[] bytes = contentSource.content(queued.objectId())
                    .orElseThrow(() -> new IllegalStateException(
                            "No raw content is available for analysis."));
            String text = new String(bytes, StandardCharsets.UTF_8);
            String contentSha256 = Digests.sha256Hex(bytes);

            if (reusePriorWork(queued.objectId(), contentSha256)) {
            ...
            SemanticContext context = new SemanticContext(
                    queued.objectId(), text, contentSha256, queued.level(),
                    ENGINE_VERSION, recordStore.nextDnaVersion(queued.objectId()),
                    clock.instant());
            ...
            cache.put(new AnalysisCache.CacheEntry(
                    queued.objectId(), contentSha256, ENGINE_VERSION,
